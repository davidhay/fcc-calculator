    ...
    setState((oldState) => {
      if (oldState.showingResult) {
        ...
        if (isOperator(oldState.currentVal)) {
          //current operator - we will ignore this
          if (
            oldState.items.length > 0 &&
            isOperator(oldState.items[oldState.items.length])
          ) {
            allItems = oldState.items.slice(0, oldState.items.length - 1);
          } else {
            allItems = oldState.items;
          }
        } else {
          allItems = [...oldState.items, toNumber(oldState.currentVal)];
        }
    ...
    setState((oldState) => {
      ...
      if (oldState.showingResult) {
        ...
        return {
          ...oldState,
          showingResult: false,
          items: [toNumber(oldState.currentVal)],
          currentVal: op,
        };
      } else {
        if (isOperator(oldState.currentVal)) {
          ...
          //we might have to replace operator and - with new op
          if (
            oldState.currentVal === "-" &&
            oldState.items.length > 0 &&
            ...
            return {
              ...oldState,
              showingResult: false,
              items: [...oldState.items.slice(0, oldState.items.length - 1)],
              currentVal: op,
            };
          }
  ...
  const handleNumber = (e) => {
    setState((oldState) => {
      ...
      if (oldState.showingResult) {
        ...
        //PREVIOUS NUMBER - could include '.'
        if (oldState.currentVal === "0") {
          return {
            ...oldState,
            showingResult: false,
            currentVal: num,
